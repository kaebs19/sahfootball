...
// ── app-ads.txt ─────────────────────────────────────────────────
//
// إعلان ملكية: AdMob تزحف إلى «موقع المطوّر» المكتوب في صفحة
// التطبيق على App Store و Google Play، وتقرأ هذا الملف لتعرف من
// يحقّ له بيع مساحاتنا الإعلانية. بدونه يبقى مخزوننا «غير مصرّح
// به» عند المشترين، فيُستبعد من المزادات وتنهار العائدية — وهو
// ما تقوله لوحة AdMob الآن (404 على /app-ads.txt).
//
// مسارٌ في السيرفر لا ملف في web/: الموقع كله يُخدم من هنا، وملفٌ
// على القرص يعني خطوة نشر إضافية تُنسى. والمعرّف قابل للضبط من
// البيئة كي لا يحتاج تغيير الحساب نشرةَ كود.
//
// النصّ بصيغة IAB Tech Lab: المجال، ثم معرّف الناشر، ثم DIRECT
// (نبيع مباشرة لا عبر وسيط)، ثم معرّف TAG لجوجل.
//
// كاش ساعة: الملف شبه ثابت، والزاحف يمرّ كل بضع ساعات، وملفٌ
// قديم مخبّأ هنا يعني فشل تحقّق لا يُفسَّر.
const ADMOB_PUB_ID = String(process.env.ADMOB_PUB_ID || 'pub-8219247197168750').trim();
const APP_ADS_TXT = `google.com, ${ADMOB_PUB_ID}, DIRECT, f08c47fec0942fa0\n`;
