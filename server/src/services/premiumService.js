...
const DEFAULT_PREMIUM = {
  // enabled = الشراء يعمل فعلاً (StoreKit/Play Billing موصولان والسيرفر
  // يتحقق من الإيصالات). visible = تُعرض صفحة التاج ومداخلها ولو كان
  // الشراء مغلقاً: مزايا وأسعار وعبارة «قريباً» بلا زرّ يفشل.
  //
  // مفتاحان لا واحد لأن السؤالين مختلفان: «هل نُري اللاعب ما سيأتي؟»
  // قرار تسويقي، و«هل نأخذ ماله؟» قرار تقني يتوقف على ربط المتاجر.
  // وحين كان المفتاح واحداً صار فتحُ الصفحة للعرض يعني زرّ شراء يردّ
  ...
  crown: {
    ...
    // السعر هنا احتياطيٌّ يُعرض قبل أن يصل سعر المتجر (آبل تشترط
    // عرض سعرها هي بعملة المشتري)، فيجب أن يطابق سعر App Store
    // للسوق السعودي بالضبط: 14.99 ر.س = شريحة $3.99. كان 19 —
    // رقمٌ لا وجود له في المتجر، ولو فشل تحميل المنتجات مرة لعرضه
    // التطبيق على الزرّ بينما يقبض المتجر غيره.
    price: 14.99,
    currency: 'SAR',
    days: 30,
    monthly_boosters: 3,
...
async function forUser(userId) {
  ...
  return {
    ...
    // هل باب الاشتراك مفتوح أصلاً؟ الرد الأول الذي يقرأه التطبيق،
    // وعليه يقرّر إن كان يُظهر «التاج الذهبي» في الإعدادات وبطاقات
    // الدعوة وزرّ الاشتراك في شيت التوقّع — أو يخفيها كلها.
    //
    // السبب أن المتجر مغلق حتى تُوصَل مشتريات المتاجر الحقيقية، وشاشة
    // أسعار بأزرار معطّلة سببٌ جاهز لرفض المراجعة في App Store
    // (بند 2.1 اكتمال التطبيق، و3.1.1 الشراء داخل التطبيق). وإخفاؤها
    // من هنا قرار لحظي لا يحتاج نشراً جديداً حين يُفتح الباب.
    store: {
      // open = تُعرض الصفحة ومداخلها. purchasable = أزرار الشراء تعمل.
      open: Boolean(cfg.enabled || cfg.visible),
      purchasable: Boolean(cfg.enabled),
    },
