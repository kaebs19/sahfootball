...
السيرفر يدعم `resend` و`mailgun` جاهزين — بلا أي حزمة إضافية:

```bash
MAIL_DRIVER=resend
RESEND_API_KEY=re_...
MAIL_FROM="ملك التوقعات <no-reply@yourdomain.com>"
```

نطاق `MAIL_FROM` يجب أن يكون موثّقاً لدى المزوّد (سجلات DNS)، وإلا
رُفضت كل رسالة. تحقق قبل أن يحتاجه مستخدم:

```bash
node scripts/sendTestMail.js you@example.com
```

السيرفر يرفض الإقلاع لو كان `MAIL_DRIVER` اسماً مجهولاً أو ينقصه
مفتاحه — لأن هذا الخطأ لا يظهر إلا حين يطلب مستخدم استعادة كلمته.
