    ...
    final ad = BannerAd(
      ...
      listener: BannerAdListener(
        ...
        onAdFailedToLoad: (ad, error) {
          // **فشل التحديث ليس فشل التحميل.**
          //
          // بانر AdMob يجدّد نفسه تلقائياً (كل ٦٠ ثانية افتراضياً
          // في إعدادات الوحدة)، وكل تجديد فاشل يستدعي هذا المستمع
          // نفسه. وكان يتلف الإعلان المعروض ويصفّره — فيظهر
          // البانر دقيقة ثم يختفي إلى الأبد عند أول تجديد لا
          // يجد مخزوناً. وهو ما رآه Mohammed: «يظهر ويختفي».
          //
          // والإعلان المعروض يبقى معروضاً: المزوّد نفسه يُبقيه
          // ويحاول مرة أخرى في الدورة التالية.
          if (_loaded) return;

          // أما فشل أول تحميل فحالة عادية لا عطل: جهاز بلا
          // إعلانات متاحة، أو شبكة، أو نفاد مخزون. نتخلّص منه
          // ونصمت — وإعادة المحاولة في حلقة تستنزف البطارية.
          ad.dispose();
          if (!mounted) return;
          setState(() {
            _ad = null;
  ...
  void _load(String unit) {
    final ad = NativeAd(
      ...
      listener: NativeAdListener(
        ...
        onAdFailedToLoad: (ad, error) {
          // نفس قاعدة البانر: المعروض يبقى، وفشلُ التجديد لا
          // يُتلفه. وهنا أثره أسوأ: اختفاء بطاقة من وسط قائمة
          // يقفز بالمحتوى تحت إصبع من يقرأ.
          if (_loaded) return;
          ad.dispose();
          if (!mounted) return;
          setState(() {
